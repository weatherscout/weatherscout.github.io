/**
 * WeatherScout - Severe Alerts & SPC Ingestion Module
 */
...
window.spcSources = [
    { id: "spc-day1-cat", url: "https://www.spc.noaa.gov/products/outlook/day1otlk_cat.nolyr.geojson" },
    { id: "spc-day1-torn", url: "https://www.spc.noaa.gov/products/outlook/day1otlk_torn.nolyr.geojson" },
    { id: "spc-day1-hail", url: "https://www.spc.noaa.gov/products/outlook/day1otlk_hail.nolyr.geojson" },
    { id: "spc-day1-wind", url: "https://www.spc.noaa.gov/products/outlook/day1otlk_wind.nolyr.geojson" },
    { id: "spc-day2-cat", url: "https://www.spc.noaa.gov/products/outlook/day2otlk_cat.nolyr.geojson" },
    { id: "spc-day2-torn", url: "https://www.spc.noaa.gov/products/outlook/day2otlk_torn.nolyr.geojson" },
    { id: "spc-day2-hail", url: "https://www.spc.noaa.gov/products/outlook/day2otlk_hail.nolyr.geojson" },
    { id: "spc-day2-wind", url: "https://www.spc.noaa.gov/products/outlook/day2otlk_wind.nolyr.geojson" },
    { id: "spc-day3-cat", url: "https://www.spc.noaa.gov/products/outlook/day3otlk_cat.nolyr.geojson" },
    { id: "spc-day3-prob", url: "https://www.spc.noaa.gov/products/outlook/day3otlk_prob.nolyr.geojson" },
    { id: "spc-day4-prob", url: "https://www.spc.noaa.gov/products/exper/day4-8/day4prob.nolyr.geojson" },
    { id: "spc-day5-prob", url: "https://www.spc.noaa.gov/products/exper/day4-8/day5prob.nolyr.geojson" },
    { id: "spc-day6-prob", url: "https://www.spc.noaa.gov/products/exper/day4-8/day6prob.nolyr.geojson" },
    { id: "spc-day7-prob", url: "https://www.spc.noaa.gov/products/exper/day4-8/day7prob.nolyr.geojson" },
    { id: "spc-day8-prob", url: "https://www.spc.noaa.gov/products/exper/day4-8/day8prob.nolyr.geojson" }
];
...
window.getSpecificAlertName = function(props) {
    ...
    const params = props.parameters || {};
    let watchSuffix = "";
    const isConvectiveWatch = event === "Tornado Watch" || event === "Severe Thunderstorm Watch";
...
window.getAlertPriorityScore = function(feature) {
    ...
    const dmg = (params.tornadoDamageThreat?.[0] || params.thunderstormDamageThreat?.[0] || params.flashFloodDamageThreat?.[0] || "").toUpperCase();
    const det = (params.tornadoDetection?.[0] || params.waterspoutDetection?.[0] || "").toUpperCase();
    
    if (event === "Tornado Warning") { if (dmg === "CATASTROPHIC") return 1; if (dmg === "CONSIDERABLE") return 2; return 3; }
    if (event === "Severe Thunderstorm Warning") { if (dmg === "DESTRUCTIVE") return 4; if (dmg === "CONSIDERABLE") return 5; if (det === "POSSIBLE") return 6; return 7; }
    if (event === "Extreme Wind Warning") return 8; if (event === "Tsunami Warning") return 9; if (event === "Typhoon Warning") return 10; if (event === "Hurricane Warning") return 11; if (event === "Hurricane Force Wind Warning") return 12; if (event === "Storm Warning") return 13; if (event === "Tropical Storm Warning") return 14; if (event === "Typhoon Watch") return 15; if (event === "Hurricane Watch") return 16; if (event === "Hurricane Force Wind Watch") return 17; if (event === "Storm Surge Watch") return 18; if (event === "Tropical Storm Watch") return 19; if (event === "Tropical Cyclone Local Statement") return 20; if (event === "Flash Flood Warning") { if (dmg === "CATASTROPHIC") return 21; if (dmg === "CONSIDERABLE") return 22; return 23; }
    if (event === "Blizzard Warning") return 24; if (event === "Ice Storm Warning") return 25; if (event === "Snow Squall Warning") return 26; if (event === "Lake Effect Snow Warning") return 27; if (event === "Winter Storm Warning") return 28; if (event === "Heavy Freezing Spray Warning") return 29; if (event === "Extreme Heat Warning") return 30; if (event === "Extreme Cold Warning") return 31; if (event === "Dust Storm Warning") return 32; if (event === "Blowing Dust Warning") return 33; if (event === "High Wind Warning") return 34; if (event === "Red Flag Warning") return 35; if (event === "Extreme Fire Danger") return 36; if (event === "Fire Warning") return 37; if (event === "Avalanche Warning") return 38; if (event === "Earthquake Warning") return 39; if (event === "Volcano Warning") return 40; if (event === "Ashfall Warning") return 41;
    if (event === "Special Marine Warning") { if (det === "OBSERVED") return 42; if (det === "POSSIBLE") return 43; return 44; }
    if (event === "Hazardous Seas Warning") return 45; if (event === "Storm Warning") return 46; if (event === "Gale Warning") return 47; if (event === "High Surf Warning") return 48; if (event === "Coastal Flood Warning") return 49; if (event === "Lakeshore Flood Warning") return 50; if (event === "Flood Warning") return 51; if (event === "Freeze Warning") return 52; if (event === "Tornado Watch") return 53; if (event === "Severe Thunderstorm Watch") return 54; if (event === "Tsunami Watch") return 55; if (event === "Flash Flood Watch") return 56; if (event === "Winter Storm Watch") return 57; if (event === "Avalanche Watch") return 58; if (event === "Heavy Freezing Spray Watch") return 59; if (event === "High Wind Watch") return 60; if (event === "Extreme Heat Watch") return 61; if (event === "Extreme Cold Watch") return 62; if (event === "Fire Weather Watch") return 63; if (event === "Hazardous Weather Outlook") return 64; if (event === "Hazardous Seas Watch") return 65; if (event === "Storm Watch") return 66; if (event === "Gale Watch") return 67; if (event === "Coastal Flood Watch") return 68; if (event === "Lakeshore Flood Watch") return 69; if (event === "Flood Watch") return 70; if (event === "Freeze Watch") return 71; if (event === "Tsunami Advisory") return 72; if (event === "Flash Flood Statement") return 73; if (event === "Severe Weather Statement") return 74; if (event === "Special Weather Statement") return 75; if (event === "Wind Advisory") return 76; if (event === "Blowing Dust Advisory") return 77; if (event === "Dust Advisory") return 78; if (event === "Heat Advisory") return 79; if (event === "Cold Weather Advisory") return 80; if (event === "Winter Weather Advisory") return 81; if (event === "Frost Advisory") return 82; if (event === "Freezing Fog Advisory") return 83; if (event === "Freezing Spray Advisory") return 84; if (event === "Dense Fog Advisory") return 85; if (event === "Dense Smoke Advisory") return 86; if (event === "Air Quality Alert") return 87; if (event === "Air Stagnation Advisory") return 88; if (event === "Flood Statement") return 89; if (event === "Lakeshore Flood Statement") return 90; if (event === "Coastal Flood Statement") return 91; if (event === "Rip Current Statement") return 92; if (event === "Beach Hazards Statement") return 93; if (event === "Flood Advisory") return 94; if (event === "Coastal Flood Advisory") return 95; if (event === "Lakeshore Flood Advisory") return 96; if (event === "High Surf Advisory") return 97; if (event === "Small Craft Advisory") return 98; if (event === "Brisk Wind Advisory") return 99; if (event === "Lake Wind Advisory") return 100; if (event === "Low Water Advisory") return 101; if (event === "Ashfall Advisory") return 102; if (event === "Avalanche Advisory") return 103; if (event === "Marine Weather Statement") return 104; if (event === "Hazardous Weather Outlook") return 105; if (event === "Hydrologic Outlook") return 106; if (event === "Short Term Forecast") return 107; if (event.startsWith("Mesoscale Discussion")) return 108; if (event === "SPC Outlook") { const day = parseInt(p.spcDay); if (day === 1) return 109; if (day === 2) return 110; if (day === 3) return 111; if (day === 4) return 112; if (day === 5) return 113; if (day === 6) return 114; if (day === 7) return 115; if (day === 8) return 116; return 117; }
    return 120;
};

// --- MAP FLASHING & NAVIGATION ---
...
window.flashAlertOnMap = function(feature, duration = 7500) {
    ...
    if (window.map.getLayer("highlight-line")) window.map.setPaintProperty("highlight-line", "line-width", bw);
    
    const setOpacity = (f, l) => {
        if (window.map.getLayer("highlight-fill")) window.map.setPaintProperty("highlight-fill", "fill-opacity", f);
        if (window.map.getLayer("highlight-line")) window.map.setPaintProperty("highlight-line", "line-opacity", l);
    };
    
    const segment = 750;
    for (let t = 0; t < duration; t += segment) {
        const isOff = (t / segment) % 2 !== 0;
        const timeoutId = setTimeout(() => { setOpacity(isOff ? 0 : 0.3, isOff ? 0 : 1.0); }, t);
...
window.createAndShowAlertPopup = function(feature, source) {
    ...
    let title = (props.messageType === "Update" ? "Updated - " : "") + specificAlertName;
    let detailsHTML = "<div>";
    ...
    if (props.event === "SPC Outlook") {
        ...
        if (props.areaDesc && props.areaDesc.toUpperCase() !== "N/A") detailsHTML += `<p class="alert-popup-area-clamp"><strong>Affected:</strong> ${props.areaDesc}</p>`;
        if (params.tornadoDetection && window.formatThreatValue(params.tornadoDetection[0])) detailsHTML += `<p><strong>Tornado:</strong> ${window.formatThreatValue(params.tornadoDetection[0])}</p>`;
        if (params.waterspoutDetection && window.formatThreatValue(params.waterspoutDetection[0])) detailsHTML += `<p><strong>Waterspout:</strong> ${window.formatThreatValue(params.waterspoutDetection[0])}</p>`;
        
        const dmg = params.tornadoDamageThreat?.[0] || params.thunderstormDamageThreat?.[0] || params.flashFloodDamageThreat?.[0];
        if (dmg && window.formatThreatValue(dmg)) detailsHTML += `<p><strong>Threat:</strong> ${window.formatThreatValue(dmg)}</p>`;
        if (params.maxWindGust && params.maxWindGust[0] !== "0 MPH" && window.formatThreatValue(params.maxWindGust[0])) {
        ...
        if (params.maxHailSize && params.maxHailSize[0] !== "0.00" && window.formatThreatValue(params.maxHailSize[0])) {
        ...
        if (params.flashFloodDetection && window.formatThreatValue(params.flashFloodDetection[0])) detailsHTML += `<p><strong>Source:</strong> ${window.formatThreatValue(params.flashFloodDetection[0])}</p>`;
    }
    ...
    popup.addEventListener("click", () => {
        if (props.event === "SPC Outlook") {
            ...
            const targetType = props.spcType || (parseInt(targetDay) >= 4 ? "prob" : "cat");
            if (window.activeSpcDay !== targetDay || window.activeSpcType !== targetType) {
...
window.displayNextAlert = function() {
    ...
    if (nextItem.feature.properties.geometryType === "zone" && !window.zoneAlertsEnabled) {
        window.alertQueue.shift();
        window.displayNextAlert();
        return;
    }
    if (nextItem.type !== "spcOutlook" && window.hiddenAlertTypes.has(nextItem.feature.properties.event)) {
        window.alertQueue.shift();
        window.displayNextAlert();
        return;
    }
...
window.refreshNwsAlerts = async function(isSilent = false) {
    ...
    try {
        ...
        const keepId = (id) => currentIds.has(id) || id.startsWith("debug") || id.startsWith("md") || id.startsWith("ww");
        ...
        window.updatedAlertIds = new Set([...window.updatedAlertIds].filter(keepId));
...
window.processRawAlertFeatures = async function(raw, filterType, isSilent = false) {
    if (!window.alertsEnabled) return;
    ...
    if (filterType === "polygon") {
        poly.forEach((f) => { f.properties.priorityScore = window.getAlertPriorityScore(f); });
        if (window.map.getSource("alerts-poly")) {
            window.map.getSource("alerts-poly").setData({ type: "FeatureCollection", features: poly });
        }
...
window.fetchIemWatchFeatures = async function() {
    try {
        const res = await fetch("https://mesonet.agron.iastate.edu/api/1/spc_watch_outline.geojson");
        const data = await res.json();
        return (data.features || []).map((f) => {
            const p = f.properties;
            const num = p.num;
            const numPadded = String(num).padStart(4, "0");
            const wType = p.type === "TOR" ? "Tornado Watch" : "Severe Thunderstorm Watch";
            const geometry = f.geometry.type === "MultiPolygon" ? { type: "Polygon", coordinates: f.geometry.coordinates[0] } : f.geometry;
            return {
                type: "Feature",
                geometry,
                properties: {
...
window.updatePlacefileAlerts = async function(isSilent = false) {
    ...
    const watchFeatures = (!window.zoneAlertsEnabled && window.alertsEnabled) ? await window.fetchIemWatchFeatures() : [];
    
    const liveIds = new Set([...mdFeatures.map((f) => f.properties.id), ...watchFeatures.map((f) => f.properties.id)]);
    window.knownAlertIds = new Set([...window.knownAlertIds].filter((id) => (!id.startsWith("md") && !id.startsWith("ww")) || liveIds.has(id)));
    window.newAlertIds = new Set([...window.newAlertIds].filter((id) => (!id.startsWith("md") && !id.startsWith("ww")) || liveIds.has(id)));
    
    if (window.alertsEnabled && window.mesoDiscussionsEnabled) {
        mdFeatures.forEach((f) => { f.properties.priorityScore = 108; });
        if (window.map.getSource("alerts-md")) {
            window.map.getSource("alerts-md").setData({ type: "FeatureCollection", features: mdFeatures });
        }
        ...
        if (window.map.getSource("alerts-md")) {
            window.map.getSource("alerts-md").setData({ type: "FeatureCollection", features: [] });
        }
        window.globalMdAlerts = [];
    }
    
    if (!window.zoneAlertsEnabled && window.alertsEnabled) {
        watchFeatures.forEach((f) => { f.properties.priorityScore = 150; });
        if (window.map.getSource("alerts-poly-watch")) {
            window.map.getSource("alerts-poly-watch").setData({ type: "FeatureCollection", features: watchFeatures });
        }
        window.globalPolyWatchAlerts = watchFeatures;
        watchFeatures.forEach((f) => window.addNewAlertToQueue(f, "alert", isSilent));
    } else {
        if (window.map.getSource("alerts-poly-watch")) {
            window.map.getSource("alerts-poly-watch").setData({ type: "FeatureCollection", features: [] });
        }
        window.globalPolyWatchAlerts = [];
    }
    window.displayNextAlert();
...
window.processZoneAlerts = async function(features, isSilent = false) {
    ...
    const tasks = features.flatMap((f) => (f.properties.affectedZones || []).map((url) => ({ url, parentFeature: f })));
    ...
    for (let i = 0; i < toFetch.length; i += 25) {
        ...
        await Promise.all(chunk.map(async (task) => {
            try {
                ...
                let parsedUrl;
                try { parsedUrl = new URL(task.url); } catch (e) { return; }
                const res = await fetch(parsedUrl.href);
                if (!res.ok) throw new Error();
                const data = await res.json();
                
                if (data.geometry && data.geometry.coordinates) {
                    let geoms = data.geometry.type === "Polygon" ? [data.geometry.coordinates] : data.geometry.coordinates;
                ...
                // Fail silently on single zone resolution failure
            }
        }));
    }
    ...
    if (window.map.getSource("alerts-zone")) {
        window.map.getSource("alerts-zone").setData({ type: "FeatureCollection", features: resolvedFeatures });
    }
    window.globalZoneAlerts = resolvedFeatures;
};

// --- SPC OUTLOOK LAYER PROCESSING ---
...
window.getHighestSpcCatFeature = function(features) {
    ...
    for (const f of features) {
        const label = (f.properties.LABEL || f.properties.LABEL2 || "").toUpperCase();
...
window.spcGetIssueValue = function(feature) {
    return (feature.properties.ISSUE || feature.properties.ISSUE_ISO || "").toString();
};

window.updateSpcOutlooks = async function() {
    await Promise.all(window.spcSources.map(async (s) => {
        ...
        try {
            ...
            const newLabels = Object.keys(newSnapshot).filter((lbl) => !(lbl in prevSnapshot));
            const updatedLabels = Object.keys(newSnapshot).filter((lbl) => lbl in prevSnapshot && newSnapshot[lbl] !== prevSnapshot[lbl]);
            if (newLabels.length === 0 && updatedLabels.length === 0) return;
            
            let topFeature = isCat ? window.getHighestSpcCatFeature(data.features) : data.features.reduce((best, f) => {
                const val = parseFloat(f.properties.LABEL || f.properties.LABEL2 || 0);
                const bestVal = parseFloat(best?.properties.LABEL || best?.properties.LABEL2 || 0);
                return val > bestVal ? f : best;
            }, null);
            if (!topFeature) return;
            
            const topLabel = (topFeature.properties.LABEL || topFeature.properties.LABEL2 || "").toUpperCase();
            ...
            window.alertQueue.push({ feature: syntheticFeature, type: "spcOutlook", score });
            window.alertQueue.sort((a, b) => a.score - b.score);
            window.displayNextAlert();
        } catch (e) {
...
window.getSpcSourceHighest = function(sourceId) {
    ...
    if (isCat) {
        ...
        if (!best) return null;
        const lbl = (best.properties.LABEL || best.properties.LABEL2 || "").toUpperCase();
        if (!lbl) return null;
        ...
        if (!best) return null;
        const val = parseFloat(best.properties.LABEL || best.properties.LABEL2 || 0);
        if (!val || val <= 0) return null;
        return { label: window.formatSpcLabel(String(best.properties.LABEL || best.properties.LABEL2)), fill: best.properties.fill || "#808080" };
    }
};
    ...
    if (window.activeSpcDay !== "none") {
        ...
        if (dayObj && dayObj[window.activeSpcType]) {
            ...
            if (window.map.getLayer(`${layerId}-border`)) {
                window.map.setLayoutProperty(`${layerId}-border`, "visibility", "visible");
            }
        }
    }
};

// --- ALERT FILTER APPLICATIONS ---
window.applyMapAlertFilters = function() {
    const hiddenArr = [...window.hiddenAlertTypes];
    const polyFilter = hiddenArr.length > 0 ? ["all", ["==", "geometryType", "polygon"], ["!in", "event", ...hiddenArr]] : ["==", "geometryType", "polygon"];
    const zoneFilter = hiddenArr.length > 0 ? ["all", ["==", "geometryType", "zone"], ["!in", "event", ...hiddenArr]] : ["==", "geometryType", "zone"];
    const openFilter = hiddenArr.length > 0 ? ["!in", "event", ...hiddenArr] : null;
    ...
    if (countEl) {
        countEl.textContent = window.hiddenAlertTypes.size > 0 ? window.hiddenAlertTypes.size : "";
    }
};
...
window.toggleAllAlerts = async function() {
    ...
    if (window.alertsEnabled) {
        ...
        ["alerts-poly", "alerts-zone", "alerts-md", "alerts-poly-watch"].forEach((s) => {
            if (window.map.getSource(s)) window.map.getSource(s).setData({ type: "FeatureCollection", features: [] });
        });
        window.globalPolyAlerts = [];
        window.globalZoneAlerts = [];
        window.globalMdAlerts = [];
    ...
    [
        window.layerIds.alertsZone, window.layerIds.alertsZoneBorder,
        window.layerIds.alertsPolygon, window.layerIds.alertsPolygonBorder,
        window.layerIds.alertsMd, window.layerIds.alertsMdBorder,
        window.layerIds.alertsPolyWatch, window.layerIds.alertsPolyWatchBorder
    ].forEach((l) => {
        if (window.map.getLayer(l)) window.map.setLayoutProperty(l, "visibility", v);
    });
    window.updateGreenStatusIndicators();
    if (window.renderAlertsSidebar) {
        window.renderAlertsSidebar();
